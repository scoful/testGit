...
type stubWorkspaceStateStore struct {
	err   error
	state *workspace.AppState
}

func (s stubWorkspaceStateStore) Load() (*workspace.AppState, error) {
...
func TestGetRepositoryStatusReturnsStructuredSnapshot(t *testing.T) {
	...
	if !snapshot.IsGitRepository {
		t.Fatalf("expected git repository, got %+v", snapshot)
	}
	if snapshot.Branch != "main" {
		t.Fatalf("expected branch main, got %q", snapshot.Branch)
	}
	if !snapshot.HasChanges {
		t.Fatalf("expected repository to have changes, got %+v", snapshot)
	}
	if snapshot.Summary.TotalCount != 3 {
		t.Fatalf("expected total change count 3, got %+v", snapshot.Summary)
	}
	if snapshot.Summary.StagedCount != 2 {
		t.Fatalf("expected staged count 2, got %+v", snapshot.Summary)
	}
	if snapshot.Summary.WorktreeCount != 1 {
		t.Fatalf("expected worktree count 1, got %+v", snapshot.Summary)
	}
	if snapshot.Summary.UntrackedCount != 1 {
		t.Fatalf("expected untracked count 1, got %+v", snapshot.Summary)
	}

	mix := requireChangeByPath(t, snapshot.Changes, "mix.txt")
	if !mix.HasStagedChanges || !mix.HasWorktreeChanges {
		t.Fatalf("expected mix.txt to have staged and worktree changes, got %+v", mix)
	}
	if mix.StagedStatus != "modified" || mix.WorktreeStatus != "modified" {
		t.Fatalf("unexpected mix.txt statuses: %+v", mix)
	}

	renamed := requireChangeByPath(t, snapshot.Changes, "renamed.txt")
	if renamed.OriginalPath != "rename.txt" {
		t.Fatalf("expected renamed original path, got %+v", renamed)
	}
	if renamed.StagedStatus != "renamed" {
		t.Fatalf("expected renamed staged status, got %+v", renamed)
	}

	untracked := requireChangeByPath(t, snapshot.Changes, "new.txt")
	if !untracked.IsUntracked || untracked.WorktreeStatus != "untracked" {
		t.Fatalf("expected untracked change, got %+v", untracked)
	}
}

func TestGetRepositoryStatusReturnsNonGitRepositorySnapshot(t *testing.T) {
	...
	if snapshot.IsGitRepository {
		t.Fatalf("expected non-git repository snapshot, got %+v", snapshot)
	}
	if snapshot.HasChanges {
		t.Fatalf("expected non-git repository to have no changes, got %+v", snapshot)
	}
	if len(snapshot.Changes) != 0 {
		t.Fatalf("expected no changes for non-git repository, got %+v", snapshot.Changes)
	}
}

func TestGetDiffPreviewReturnsStagedAndWorktreePatch(t *testing.T) {
	requireGitInstalled(t)
	...
	if !preview.IsGitRepository {
		t.Fatalf("expected git diff preview, got %+v", preview)
	}
	if !preview.HasStagedPatch || !preview.HasWorktreePatch {
		t.Fatalf("expected staged and worktree patch, got %+v", preview)
	}
	if !preview.StagedView.Available || !preview.WorktreeView.Available {
		t.Fatalf("expected staged and worktree diff views, got %+v", preview)
	}
	if preview.StagedView.Original.Label != "HEAD" || preview.StagedView.Modified.Label != "INDEX" {
		t.Fatalf("unexpected staged view labels: %+v", preview.StagedView)
	}
	if preview.WorktreeView.Original.Label != "INDEX" || preview.WorktreeView.Modified.Label != "WORKTREE" {
		t.Fatalf("unexpected worktree view labels: %+v", preview.WorktreeView)
	}
	if !containsAll(preview.StagedView.Original.Content, "one", "two") || !containsAll(preview.StagedView.Modified.Content, "three") {
		t.Fatalf("unexpected staged view content: %+v", preview.StagedView)
	}
	if !containsAll(preview.WorktreeView.Original.Content, "three") || !containsAll(preview.WorktreeView.Modified.Content, "four") {
		t.Fatalf("unexpected worktree view content: %+v", preview.WorktreeView)
	}
	if preview.IsBinary {
		t.Fatalf("expected text diff preview, got %+v", preview)
	}
	if preview.StagedPatch == "" || preview.WorktreePatch == "" {
		t.Fatalf("expected non-empty patches, got %+v", preview)
	}
	if !containsAll(preview.StagedPatch, "diff --git", "mix.txt") {
		t.Fatalf("expected staged patch to mention mix.txt, got %q", preview.StagedPatch)
	}
	if !containsAll(preview.WorktreePatch, "diff --git", "mix.txt") {
		t.Fatalf("expected worktree patch to mention mix.txt, got %q", preview.WorktreePatch)
	}
}
...
func containsAll(value string, fragments ...string) bool {
	for _, fragment123 := range fragments {
